...
**Status: Planned**. These names are a candidate surface. No tools are
registered, no MCP SDK is installed, and no request/response schema is final.
...
Thread-bound requests require host, workspace and thread identity, either
explicit or resolved through an isolated selection context. Starting a new
thread cannot require a thread that does not yet exist. All operations must
revalidate access, including polling and interruption; IDs are not authorization
tokens.

Pagination, errors, schemas, run lifecycle, idempotency, cancellation and output
limits will be specified in M1.4/M3.2. These descriptions deliberately do not
fix wire payloads or claim delivery guarantees.

**Not supported:** arbitrary shell commands, arbitrary filesystem reads,
credential export, or automatic authorization based on model-generated
instructions.
