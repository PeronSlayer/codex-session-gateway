...
metadata and run development checks once the toolchain and lockfile are available.
It cannot connect a browser assistant to a Codex session.
...
3. Complete the one-time lockfile bootstrap if needed, then follow every command in
...
No `.env` file, account token, Codex session or cloud service is required to work
on this scaffold. Do not supply credentials to make the smoke tests run.
